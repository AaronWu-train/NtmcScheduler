...
\paragraph{Daily unique assignment (\texttt{GEN-H-01})}
...
\paragraph{Legal working stations (\texttt{M-H-01})}

若車站 \(l\notin\mathcal{A}_e\)，則 \(x_{edls}=0\)。因此人員只能在本人所屬
車站或同群組車站工作，不得跨群組。

\paragraph{Coverage and external-staffing restriction (\texttt{M-H-02}, \texttt{M-H-03})}
...
\paragraph{Minimum eleven-hour rest (\texttt{GEN-H-03})}
...
\paragraph{At most six consecutive days without a general rest (\texttt{GEN-H-02})}
...
\paragraph{Eight-week general rest quota (\texttt{GEN-H-04})}
...
\paragraph{Eight-week national-holiday rest quota (\texttt{GEN-H-04})}
...
\paragraph{Fixed events and historical state (\texttt{GEN-H-05})}
...
\paragraph{Requested rest (\texttt{GEN-R-01})}
...
\paragraph{Monthly general-rest distribution}
...
\paragraph{Monthly national-holiday-rest distribution}
...
\paragraph{External staffing (\texttt{M-S-EXT})}
...
\paragraph{Non-home-station assignments (\texttt{M-S-HOME})}
...
\paragraph{Continuous work-streak length (\texttt{GEN-S-STREAK})}
...
\paragraph{Same-shift block length (\texttt{M-S-BLOCK})}
...
\paragraph{Night--rest--early pattern (\texttt{M-S-NIGHT-EARLY})}
...
\paragraph{Night--rest--afternoon pattern (\texttt{M-S-NIGHT-AFTERNOON})}

同理，令 \(\pi^{NA}_{ed}=1\) 若且唯若日期 \(d\) 為夜班、日期 \(d+1\) 為任一
種實際休假、日期 \(d+2\) 為午班。總違反量為
\(\Phi^{NA}=\sum_{e,d}\pi^{NA}_{ed}\)。

\paragraph{Shift change without rest (\texttt{M-S-RESTSWITCH})}
...
\paragraph{Preferred shift rotation (\texttt{M-S-ROTATE})}
...
\paragraph{Weekday rest fairness (\texttt{GEN-S-WEEKDAY-R})}
...
\paragraph{Holiday rest fairness (\texttt{GEN-S-WEEKEND-R})}
...
\paragraph{Cross-station support fairness (\texttt{M-S-SUPPORT-FAIR})}
...
\[
  \begin{aligned}
    \Phi^{R^*}        &:\text{未滿足指定休假},\\
    \Phi^{month,R}    &:\text{本月一般 }R\text{ 數量偏離},\\
    \Phi^{month,R1}   &:\text{本月 }R1\text{ 數量偏離},\\
    \Phi^{ext}        &:\text{外派班位},\\
    \Phi^{home}       &:\text{非本站工作},\\
    \Phi^{streak}     &:\text{連續工作區段長度},\\
    \Phi^{block}      &:\text{同班別區塊長度},\\
    \Phi^{NE}         &:\text{夜--休--早},\\
    \Phi^{NA}         &:\text{夜--休--午},\\
    \Phi^{restswitch} &:\text{未經休假換班},\\
    \Phi^{rotate}     &:\text{非偏好輪轉},\\
    \Phi^{weekday}    &:\text{平日休假公平},\\
    \Phi^{holiday}    &:\text{假日休假公平},\\
    \Phi^{support}    &:\text{跨站支援公平}.
  \end{aligned}
\]
...
\begin{longtable}{p{3.0cm}p{3.0cm}p{4.4cm}p{5.0cm}}
  ...
  本月一般 \(R\) & 超出 \(\pm1\) 後的平方懲罰 & 理論上至 \(N\max_q f_n(q)\) & 合法班表通常為 0 至數十；極端值可達數千以上，平方項可能主導同組目標。 \\
  本月 \(R1\) & 超出 \(\pm1\) 後的平方懲罰 & 理論上至 \(N\max_q f_m(q)\) & 通常小於一般 \(R\)，但仍應避免與純計數規則直接使用相同權重。 \\
...
\paragraph{Fixed monthly shift (\texttt{T-H-01})}
...
\paragraph{Eight-week rest quotas (\texttt{GEN-H-04})}
...
\paragraph{Shift attendance (\texttt{T-S-ATTEND})}
...
\paragraph{Specialty attendance (\texttt{T-S-SPECIALTY})}
...
\paragraph{Ability level (\texttt{T-S-ABILITY})}
...
\paragraph{Night-to-early month transition rest (\texttt{T-S-MONTH-REST})}
...
\paragraph{Month-boundary rest balance (\texttt{T-S-MONTH-BALANCE})}
...
\[
  \begin{aligned}
    \Phi^{T,R^*}          &: \text{未滿足指定休假},\\
    \Phi^{T,month,R}      &: \text{本月一般 }R\text{ 數量偏離},\\
    \Phi^{T,month,R1}     &: \text{本月 }R1\text{ 數量偏離},\\
    \Phi^{T,att}          &: \text{班組出勤不足},\\
    \Phi^{T,specialty}    &: \text{專業組別缺席},\\
    \Phi^{T,ability}      &: \text{平均能力不足},\\
    \Phi^{T,streak}       &: \text{連續工作區段長度},\\
    \Phi^{T,monthrest}    &: \text{夜班轉早班休假不足},\\
    \Phi^{T,monthbalance} &: \text{月交界休假過度集中},\\
    \Phi^{T,weekday}      &: \text{平日休假公平},\\
    \Phi^{T,holiday}      &: \text{假日休假公平}.
  \end{aligned}
\]
...
\begin{longtable}{p{2.8cm}p{2.5cm}p{4.5cm}p{4.3cm}}
  ...
  本月一般 \(R\) & 超出 \(\pm1\) 後的平方懲罰 & 理論上至 \(N_T\max_q f_n(q)\) & 平方項可能快速放大，不宜直接與純計數規則使用相同權重。 \\
  本月 \(R1\) & 超出 \(\pm1\) 後的平方懲罰 & 理論上至 \(N_T\max_q f_m(q)\) & 原始值通常小於一般 \(R\)，但仍需注意平方尺度。 \\
  班組出勤不足 & 缺少人數 & 上限為 \(\sum_{d,s}B_{ds}\)，約不超過 \(DN_T/2\) & 是人數缺口，單位意義直接。 \\
  專業組別缺席 & 班別--日期--專業 & 上限為 \(\sum_{d,s}|\mathcal{Q}_{ds}|\leq3DQ_{\max}\) & 專業種類愈多，原始值愈大。 \\
  能力不足 & 能力分數 & \(0\leq\Phi^{T,ability}\leq2DN_T\) & 與人數型違反量尺度不同，宜獨立分組或縮小權重。 \\
  連續工作區段 & 區段偏離值 & 約不超過 \(4N_T\lceil D/2\rceil\) & 實際通常遠低於粗略上限。 \\
  夜轉早休假不足 & 缺少休假日 & \(0\leq\Phi^{T,monthrest}\leq2|\mathcal{E}^{N\rightarrow E}|\) & 原始值很小，與大型總和同組時需提高相對權重。 \\
