...
決定每位人員每日的工作班別或休假狀態，包括每人指定數量的 \(R休\)。
...
滿足，求解器仍須決定該日實際使用一般 \(R\)、\(R1\) 或 \(R休\)。令
\(k_e\in\mathbb Z_{\geq0}\) 為人員 \(e\) 在目標月份必須使用的精確 \(R休\) 數，
取自需求列的 \texttt{當月指定R休}；歷史與候選列的同名欄位則記錄日格實際 \(R休\) 數。
...
承接跨期工作區段的班型一致性或其他軟規則狀態，資料還應向前涵蓋至相關未結束區段的
...
\begin{itemize}
  ...
  \item \texttt{LB01早}、\texttt{LB02午}、\texttt{LB06夜}：正常班，無論是否在
    本站工作都明確寫出車站與班別。
  \item \texttt{R}：一般休假；\texttt{R1}：國定假日額度休假；\texttt{R休}：已決定的歷史自選休假。
  \item \texttt{R*}：本期休假申請，實際使用 \(R\)、\(R1\) 或 \(R休\) 由求解器決定。
  \item \texttt{R*[R]}、\texttt{R*[R1]}、\texttt{R*[R休]}：已滿足的休假申請，方括號內保存其
...
\[
  ...
  r^{L}_{ed}\in\{0,1\},
\]
分別表示一般 \(R\)、\(R1\) 與 \(R休\)。\(R^*\) 不建立獨立狀態變數；其是否被滿足，
...
\[
  ...
  \qquad
  \rho_{ed}=r_{ed}+r^{1}_{ed}+r^{L}_{ed}.
\]
...
\[
  r^{L}_{ed}\leq p_{ed},
  \qquad
  \sum_{d\in\mathcal D^M}r^{L}_{ed}=k_e.
\]
因此 \(R休\) 只能使用目標月的 \(R^*\)，不延伸至月底後七日，也不計入跨月額度。
...
\paragraph{班位覆蓋與外援限制（\texttt{MH02}、\texttt{MH03}）}
...
\paragraph{八週一般 R 額度（\texttt{CH04}）}
...
\]
\(R1\) 可以安排在週期內任意日期，不必安排於國定假日當日。\(R休\) 不計入一般 \(R\) 或 \(R1\) 的八週額度。
...
\paragraph{固定事件與歷史狀態（\texttt{CH05}）}
...
\begin{itemize}
  ...
  \item 尚未結束的連續工作區段、其中最近的正常班型及是否已混合班型；
...
\(R\)、\(R1\) 或 \(R休\)，即視為申請被滿足。因此未滿足指定休假的違反量為
\[
  \Phi^{R^*}
  =
  \sum_{\substack{e\in\mathcal{E},\ d\in\mathcal{D}^{M}\\p_{ed}=1}}
  (1-r_{ed}-r^{1}_{ed}-r^{L}_{ed}).
\]
若申請以一般 \(R\) 滿足，輸出為 \texttt{R*[R]}；若以 \(R1\) 滿足，輸出為
\texttt{R*[R1]}；若以 \(R休\) 滿足，輸出為 \texttt{R*[R休]}。

每人的 \(R休\) 輸入數量 \(k_e\) 是可使用上限；未使用量為
\[
  \Phi^{unusedL}
  =\sum_{e\in\mathcal E}\left(k_e-\sum_{d\in\mathcal D^M}r^L_{ed}\right).
\]
...
\]
因此偏離基準 0、1、2、3 日時，懲罰依序為 0、1、4、9。以下將此離散函數記為
\(f_n(q)=|q-n|^2\)。

\paragraph{八週累積 R1 餘額（\texttt{CS03}）}

對每個與目標月相交的 56 日區間 \(I\)，令 \(c_I\) 為月底與區間結束日的較早者，
\(p^1_{eI}\) 為目標月前已使用或折抵的 \(R1\) 數，並定義
\[
  A^1_{eI}=p^1_{eI}+\sum_{d\in\mathcal D^M\cap I}r^1_{ed},
  \qquad H_I(c_I)=|\{h\in\mathcal H_I:h\le c_I\}|.
\]
令
\[
  g(a,h)=\max\{0,a-h\}^2+\max\{0,h-a-1\}^2,
  \qquad
  \Phi^{balance,R1}=\sum_e\sum_I g(A^1_{eI},H_I(c_I)).
\]
累積剛好或暫欠一日皆不懲罰；提前多休與超過一日的欠額按平方計分。餘額沿同一
區間跨月承接，區間結束時仍由八週硬規則強制精確額度。
...
\[
  ...
  \end{cases}
\]
因此 M 長度 1、2、3、4、5 的懲罰分別為 4、1、0、0、0；長度 6 以上為 \(2(L-4)\)。
...
對每個區段 \(B\)，令
\[
  \mathcal S_B=\{s\in\mathcal S:\exists d\in B,\ y^M_{eds}=1\}.
\]
...
\[
  \Phi^{mixed}
  =\sum_{e\in\mathcal E}
  \sum_{B\in\mathcal B^{work}_e:\ \max B\in\mathcal D^M}\mu_B.
\]
...
\begin{itemize}
  \item 若當日為一般 \(R\)、\(R1\) 或 \(R休\)，則 \(\gamma_{ed}=0\)。
...
有效正常班別；一般 \(R\)、\(R1\)、\(R休\) 與 \(X\) 均不改變此狀態。偏好轉移集合為
\[
  \mathcal{P}
  =
...
\[
  R^{wd}_{e}
  =
  \sum_{d\in\mathcal{D}^{M}\cap\mathcal{D}^{wd}}\rho_{ed}.
\]
其中國定假日即使落在星期一至星期五，也不屬於平日集合。平日休假公平性違反量為
\[
  \Phi^{weekday}
  =
  \sum_g
  \left(
    \max_{e\in\mathcal E^{full}_g}R^{wd}_{e}
    -
    \min_{e\in\mathcal E^{full}_g}R^{wd}_{e}
  \right).
\]
...
\[
  R^{hol}_{e}
  =
  \sum_{d\in\mathcal{D}^{M}\cap\mathcal{D}^{hol}}\rho_{ed}.
\]
...
\[
  ...
  \sum_g
  \left(
    \max_{e\in\mathcal E^{full}_g}R^{hol}_{e}
    -
    \min_{e\in\mathcal E^{full}_g}R^{hol}_{e}
  \right).
\]
...
\[
  ...
  \sum_g
  \left(
    \max_{e\in\mathcal E^{full}_g}S_e
    -
    \min_{e\in\mathcal E^{full}_g}S_e
  \right).
\]
...
\[
  C^s_e=\sum_{d\in\mathcal D^M}y_{eds}.
\]
對群組 \(g\) 令 \(n_g=|\mathcal E^{full}_g|\)，則
\[
  \Phi^{shift,s}
  =\sum_g\left(n_g\sum_{e\in\mathcal E^{full}_g}(C^s_e)^2
  -\left(\sum_{e\in\mathcal E^{full}_g}C^s_e\right)^2\right).
\]
這是群組內班數變異數的整數等價量；\(X\) 與延伸日不計。
...
\[
  \begin{aligned}
    ...
    \Phi^{shift,s}    &:\text{群組內班別數公平 }(\texttt{MS09}).
  \end{aligned}
\]

考量各違反量的業務重要性與原始尺度差異，本模型採用
「組間字典序、組內加權和」的最佳化方式。M 定義三個目標群組：

\[
  J_1=3\Phi^{R^*}+\Phi^{unusedL},
\]
...
\begin{itemize}
  ...
  \item \(J_4\)：共同權衡外援、月休分布、工作區段與班別模式。
  \item \(J_5\)：最後改善非所屬站指派與群組內公平性。
\end{itemize}
...
\(J_4\) 與 \(J_5\)。因此低順位群組無法以犧牲高順位群組為代價取得改善；
...
\begin{longtable}{p{3.0cm}p{3.0cm}p{4.4cm}p{5.0cm}}
  ...
  本月一般 \(R\)（\texttt{CS02}） & 與目標差額的平方懲罰 & 理論上至 \(N\max_q f_n(q)\) & 合法班表通常為 0 至數十；極端值可達數千以上，平方項可能主導同組目標。 \\
  本月 \(R1\)（\texttt{CS03}） & 與目標差額的平方懲罰 & 理論上至 \(N\max_q f_m(q)\) & 通常小於一般 \(R\)，但仍應避免與純計數規則直接使用相同權重。 \\
  外派班位（\texttt{MS01}） & 超過 70 的班位數 & \(0\leq\Phi^{ext}\leq\max\{0,6D-70\}\) & 上限約為 110--116。 \\
  非本站工作（\texttt{MS02}） & 人日 & \(0\leq\Phi^{home}\leq ND\) & 上限約為 1080--1488。 \\
  連續工作區段（\texttt{CS04}） & 區段偏離值 & 約不超過 \(5N\lceil D/2\rceil\) & 實際通常遠低於粗略上限。 \\
  工作區段混合班型（\texttt{MS03}） & 混合班型的工作區段數 & 粗略上限約 \(N\lceil D/2\rceil\) & 與 CS04 使用相同區段邊界。 \\
...
\texttt{input.csv} 固定的正常班、\(R\)、\(R1\)、\(R休\) 或 \(X\)，以及延伸日期，均不列入
差異分母。\(R^*\) 只固定休假申請，底層使用 \(R\)、\(R1\) 或 \(R休\) 仍由求解器決定，
因此該格仍列入比較。

候選比較依實際排班狀態進行，忽略 \(R^*\) 的顯示標記：\texttt{R} 與
\texttt{R*[R]} 視為相同結果，\texttt{R1} 與 \texttt{R*[R1]} 視為相同結果；一般
\(R\)、\(R1\) 與 \(R休\) 仍視為不同狀態。若可比較格子數為 \(N_{cell}\)，兩份候選至少
...
求解器以月班組為高權重偏好，必要時可將正常工作排入其他班別並計分。T 模型沒有
M 的車站班位與外派變數；其核心營運目標是維持各班組的出勤、專業組成與能力水準。
...
儲存格語法。正常出勤直接寫為 \texttt{早}、\texttt{午} 或 \texttt{夜}；其值可以
不同於該日期的固定班組，並計入非月班別軟性違反量。\texttt{R}、\texttt{R1}、\texttt{R*}、
...
休假變數為 \(r^{T}_{ed},r^{T,1}_{ed},r^{T,L}_{ed}\in\{0,1\}\)，分別表示一般 \(R\)、
\(R1\) 與 \(R休\)。
...
\[
  ...
  \qquad
  \sum_{d\in\mathcal D^M}r^{T,L}_{ed}=k_e.
\]
...
\[
  ...
  (1-r^{T}_{ed}-r^{T,1}_{ed}-r^{T,L}_{ed}).
\]
滿足後依實際額度輸出為 \texttt{R*[R]}、\texttt{R*[R1]} 或 \texttt{R*[R休]}。
...
\]
因此偏離基準 0、1、2、3 日時，懲罰依序為 0、1、4、9。

\paragraph{八週累積 R1 餘額（\texttt{CS03}）}

T 沿用上述 \(g\)。令 \(p^{T,1}_{eI}\) 為目標月前已使用或折抵的 \(R1\) 數，則
\[
  A^{T,1}_{eI}=p^{T,1}_{eI}+\sum_{d\in\mathcal D^M\cap I}r^{T,1}_{ed},
  \qquad
  \Phi^{T,balance,R1}=\sum_e\sum_I g(A^{T,1}_{eI},H_I(c_I)).
\]

\paragraph{月班別一致性（\texttt{TS06}）}

每個模型日期排入非該日期固定班組的正常工作格計一次違反；\(X\) 不計：
\[
  \Phi^{T,nonmonthly}
  =
...
人數為
\[
  A_{ds}=\sum_{e\in\mathcal{E}^{T}}x^{T}_{eds}.
\]
...
\]
令 \(M^{sp}_{dsq}=|\{e\in\mathcal{E}^{T}:\kappa_e=q\}|\)，並以
...
連續曆日皆為正常班或 \(X\) 的最大區段；一般 \(R\)、\(R1\) 與 \(R休\) 都會結束區段。
除五日區段仍取 1 外，沿用 M 模型的區段偏離函數 \(D^M(L)\)，令 T 的
\(D^T(5)=1\)、\(D^T(L)=D^M(L)\ (L\neq5)\)。令已在目標月份內結束的每個工作區段依
...
\[
  \begin{aligned}
    ...
    \Phi^{T,nonmonthly}   &: \text{非月班別正常工作 }(\texttt{TS06}),\\
    \Phi^{T,month,R}      &: \text{本月一般 }R\text{ 數量偏離 }(\texttt{CS02}),\\
    \Phi^{T,balance,R1}   &: \text{八週累積 }R1\text{ 餘額 }(\texttt{CS03}),\\
    \Phi^{T,att}          &: \text{班組出勤不足 }(\texttt{TS01}),\\
    \Phi^{T,specialty}    &: \text{專業組別缺席 }(\texttt{TS02}),\\
    \Phi^{T,ability}      &: \text{高能力人員不足 }(\texttt{TS03}),\\
...
其中，偏離月班別與班組出勤不足在 \(J^T_2\) 中具有最高權重；專業組別缺席次之。
能力不足在每班完全沒有高能力人員時會產生 10 點懲罰，因此使用單位權重。
夜班轉早班休假不足直接影響跨月作息，故在 \(J^T_4\) 中給予較高
...
\begin{longtable}{p{2.8cm}p{2.5cm}p{4.5cm}p{4.3cm}}
  ...
  本月一般 \(R\)（\texttt{CS02}） & 與目標差額的平方懲罰 & 理論上至 \(N_T\max_q f_n(q)\) & 平方項可能快速放大，不宜直接與純計數規則使用相同權重。 \\
  本月 \(R1\)（\texttt{CS03}） & 與目標差額的平方懲罰 & 理論上至 \(N_T\max_q f_m(q)\) & 原始值通常小於一般 \(R\)，但仍需注意平方尺度。 \\
  非月班別工作（\texttt{TS06}） & 人日 & 上限為 \(N_T|\mathcal{D}|\) & 高權重，但仍可與同組營運品質交換。 \\
  ...
  能力不足（\texttt{TS03}） & 班別--日期懲罰 & \(0\leq\Phi^{T,ability}\leq30D\) & 每班至少 2 位為 0、只有 1 位為 1、完全沒有為 10。 \\
  連續工作區段（\texttt{CS04}） & 區段偏離值 & 約不超過 \(5N_T\lceil D/2\rceil\) & 實際通常遠低於粗略上限。 \\
...
\(R\)、\(R1\)、\(R休\) 或 \(X\)，以及延伸日期，均不列入差異分母。\(R^*\) 的底層休假類型
仍由求解器決定，因此該格仍列入比較。

比較時忽略 \(R^*\) 顯示標記：\texttt{R} 與 \texttt{R*[R]} 視為相同，\texttt{R1}
與 \texttt{R*[R1]} 視為相同，\texttt{R休} 與 \texttt{R*[R休]} 視為相同；三種休假仍是不同實際狀態。兩份候選的
最低差異格數為 \(\lceil0.1N_{cell}\rceil\)。

\subsubsection{Infeasibility Handling}

T 沒有 M 的班位覆蓋與外派變數，因此不建立 Shortage Analysis。若固定指派、
