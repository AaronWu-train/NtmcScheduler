...
\paragraph{每月 R1 分布（\texttt{CS03}）}

同理，令 \(Q^{R1}_e=\sum_{d\in\mathcal{D}^{M}}r^{1}_{ed}\)。以 \(m\) 為基準，定義
\[
  \Delta^{R1}_e=|Q^{R1}_e-m|,
  \qquad
  \Phi^{month,R1}=\sum_{e\in\mathcal{E}}(\Delta^{R1}_e)^2.
\]
以下將對應的離散函數記為
\(f_m(q)=|q-m|^2\)。上述平方懲罰可使用整數查表，
或以絕對值、最大值與乘法等整數限制建立。兩項月休分布皆為軟性偏好，
不取代八週週期的精確額度限制。此規則會抑制在單一月份過早使用過多週期額度，
但因其仍為軟限制，必要時不保證一定保留固定數量至後續月份。\(R休\) 不計入兩項月休分布。
...
\[
  ...
  \end{cases}
\]
因此長度 1、2、3、4、5 的懲罰分別為 4、2、0、0、1；長度 6 以上為 \(2(L-4)\)。
...
\begin{itemize}
  ...
  \item \(J_4\)：共同權衡外援、月休分布、跨站、工作區段與班別模式。
  \item \(J_5\)：最後改善群組內公平性。
\end{itemize}
