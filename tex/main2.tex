...
\]
對群組 \(g\) 令 \(n_g=|\mathcal E^{full}_g|\)，並定義
\[
  \Phi^{M,shift,s}
  =\sum_{g\in\mathcal G}
  \left(n_g\sum_{e\in\mathcal E^{full}_g}(C^{M,s}_e)^2
  -\left(\sum_{e\in\mathcal E^{full}_g}C^{M,s}_e\right)^2\right).
\]
此為群組內班數變異數的整數等價量；\(X\) 與延伸日不計。
...
\[
  ...
  +4\Phi^{M,holiday}
  +3\Phi^{M,support}
  +\Phi^{M,shift,\mathrm E}
  +\Phi^{M,shift,\mathrm A}
  +2\Phi^{M,shift,\mathrm N}.
\]
完整目標為
\[
  \operatorname{lexmin}
  \left(J^M_1,J^M_4,J^M_5\right).
\]
...
只有在該模型的所有目標群組皆證明為最優後，系統才搜尋替代候選。每個模型最多輸出
...
\begin{longtable}{p{2.7cm}p{5.2cm}p{6.4cm}}
  ...
  \(J^M_1,J^M_4,J^M_5\) & \texttt{objectives} 中的 \texttt{LinearExpr Total} & \texttt{BuildObjectiveGroups}；權重直接寫在同一函數 \\
  \bottomrule
\end{longtable}
\normalsize
