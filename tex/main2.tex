...
早、午班的必要班位至少補足；夜班不得多人，有需求時恰好一人；需求為 0 的站班
組合仍禁止排人：
\[
  \begin{cases}
...
\[
  \Phi^{M,weekday}
  =
  \sum_{g\in\mathcal{G}}F_g(R^{M,wd}).
\]
...
\[
  \Phi^{M,shift,s}
  =\sum_{g\in\mathcal G}F_g(C^{M,s}).
\]
此量計入群組內每位人員偏離群組平均的程度；\(X\) 與延伸日不計。
...
\[
  ...
  +5\Phi^{M,holiday}
  +0\Phi^{M,support}
  +2\Phi^{M,shift,\mathrm E}
  +2\Phi^{M,shift,\mathrm A}
  +10\Phi^{M,shift,\mathrm N}.
\]
實作時一般違反量的係數乘以 10，以 CP-SAT 整數係數精確表示未休假直接換班的
相對權重 (0.5)；公平性違反量 $F_g$ 已乘以 10，故保留 $J^M_5$ 所列整數係數。
兩者的加權分數皆位於相同的 10 倍尺度，不改變最佳化取捨。
令
\[
  J^M_{4+5}=J^M_4+J^M_5.
\]
完整目標為
\[
  \operatorname{lexmin}
...
每個模型最多輸出三份候選。M 在總時限大於 60 秒時預留最後 30 秒，否則預留
總時限的一半搜尋替代候選。若當前目標只取得 feasible incumbent，替代候選可變差，
但每個具名目標分數至多為第一份的 120\%。T 仍只在所有目標群組皆證明最優且
以等式固定後搜尋替代候選，因此 T 候選維持同分。
...
達到相同門檻。若第一份 M 候選的具名目標分數為 \(V_j^{(1)}\)，則每份 TLE 備援
候選 \(k\) 尚須滿足
\[
