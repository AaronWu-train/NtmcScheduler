...
\texttt{TH}/\texttt{TS} 表示 T 規則。ID 僅識別規則本身，不表示優先順序。
...
歷史班表另提供月初邊界值：法規連續日數、尚未結束的實際工作區段、尚未結束
的同班別區塊、最近一次有效正常班別，以及平日休假、假日休假與跨站支援的
週期累積值。這些值視為模型的固定初始參數。
...
\[
  ...
  \overline{R}^{wd}_{ec}
  +
  \sum_{d\in c\cap\mathcal{D}^{\mathrm{tar}}\cap\mathcal{D}^{wd}}
  \rho^{M}_{ed}.
\]
同一車站群組內的公平性違反量為
\[
  ...
  \sum_{c\in\mathcal{C}}
  \left(
    \max_{e:g(h_e)=g}R^{M,wd}_{ec}
    -
    \min_{e:g(h_e)=g}R^{M,wd}_{ec}
  \right).
\]
...
\[
  ...
  \overline{R}^{hol}_{ec}
  +
  \sum_{d\in c\cap\mathcal{D}^{\mathrm{tar}}\cap\mathcal{D}^{hol}}
  \rho^{M}_{ed}.
\]
則
\[
  ...
  \sum_{c\in\mathcal{C}}
  \left(
    \max_{e:g(h_e)=g}R^{M,hol}_{ec}
    -
    \min_{e:g(h_e)=g}R^{M,hol}_{ec}
  \right).
\]
...
\[
  ...
  \overline{S}_{ec}
  +
  \sum_{d\in c\cap\mathcal{D}^{\mathrm{tar}}}u_{ed}.
\]
則
\[
  \Phi^{M,support}
  ...
  \sum_{c\in\mathcal{C}}
  \left(
    \max_{e:g(h_e)=g}S^{M}_{ec}
    -
    \min_{e:g(h_e)=g}S^{M}_{ec}
  \right).
\]
...
\begin{itemize}
  \item 目標月份與固定延伸天數 \(K\)；
  \item T 人員、專業組別、能力分數及各月份固定班組；
  \item T 正常班別的實際起訖時間；
  \item 指定休假 \(R^*\)、固定勤務 \(X\) 與其他固定指派；
  \item 八週週期的一般 \(R\) 與 \(R1\) 額度；
  \item 目標月份的一般 \(R\) 與 \(R1\) 基準數量；
  \item 足以重建月初邊界狀態、跨月夜轉早狀態及公平性累積值的歷史班表；
  \item 國定假日集合。
\end{itemize}
...
結束後至 \(d_1\) 之前已出現的實際休假日數。歷史資料另提供法規連續日數、
尚未結束的實際工作區段，以及平日與假日休假的週期累積值。
...
\[
  ...
  \sum_{c\in\mathcal{C}}
  \left(
    \max_{e\in\mathcal{E}^{\mathrm{tar}}_s}R^{T,wd}_{ec}
    -
    \min_{e\in\mathcal{E}^{\mathrm{tar}}_s}R^{T,wd}_{ec}
  \right).
\]
...
\[
  ...
  \sum_{c\in\mathcal{C}}
  \left(
    \max_{e\in\mathcal{E}^{\mathrm{tar}}_s}R^{T,hol}_{ec}
    -
    \min_{e\in\mathcal{E}^{\mathrm{tar}}_s}R^{T,hol}_{ec}
  \right).
\]
