...
\paragraph{每月休假分布（\texttt{CS02}、\texttt{CS03}）}
令
\[
...
\[
  \Phi^{M,month,R1}
  =
  \sum_{e\in\mathcal{E}^{M}}f_{m_e}\!\left(Q^{M,R1}_e\right).
\]
偏離基準 0、1、2、3 日時，懲罰依序為 0、1、4、9。
此為月內分布偏好，不取代 \texttt{CH04} 的八週精確額度；\(R休\) 不計入兩個月內數量。
...
\subsubsection{\texorpdfstring{$J^M_5$}{J5}：公平性}
...
\subsubsection{\texorpdfstring{$J^T_3$}{J3}：每月休假分布}
...
\[
  \Phi^{T,month,R1}
  =
  \sum_{e\in\mathcal{E}^{T}}f_{m_e}\!\left(Q^{T,R1}_e\right).
\]
...
\begin{longtable}{p{2.7cm}p{5.2cm}p{6.4cm}}
  ...
  \(n_e,m_e\) & \texttt{ExpectedMonthlyRestCount(input, employee, special)} & 到職後目標月的六日數／國定假日數 \\
