...
\definecolor{NtmBlue}{HTML}{1F4E79}
\hypersetup{
...
\begin{titlepage}
  ...
  \vspace{1.2cm}
  {\LARGE\bfseries\color{NtmBlue} 新北捷運人員月排班問題\par}
  \vspace{0.4cm}
...
以 \(R^*[R]\)、\(R^*[R1]\) 或 \(R^*[R休]\) 保留申請記號與實際休假種類。
...
$Z_g=\sum_{e\in\mathcal E^{full}_g}z_e$，並定義十分之一為單位的正規化絕對偏差
\[
  F_g(z)
  =
  \left\lfloor
    \frac{10}{n_g}
    \sum_{e\in\mathcal E^{full}_g}|n_gz_e-Z_g|
  \right\rfloor.
\]
除以 $n_g$ 可消除群組人數造成的直接尺度差異；乘以 10 讓 CP-SAT 以整數保留
0.1 的解析度。人數少於 2 的群組其 $F_g(z)=0$。
...
每個模型最多輸出三份候選。M/T 都不預留固定候選時間；初始可行解與所有目標最佳化先共用每個 seed 的總時限，
...
Library 預設每 seed 為 300 秒、seed 0、8 workers。CLI 的共用參數形式為
\[
  \texttt{--search workers=N,seeds=N,seconds=N}.
\]
此參數同時適用 M/T；M 預設 4 workers、2 seeds、每 seed 180 秒，T 預設
8 workers、1 seed、300 秒。
portfolio 平行執行連續 seed，依第一候選的 Priority 字典序分數選出較佳的整批結果；
最大同時 worker 數約為 \texttt{workers} \(\times\) \texttt{seeds}。
...
初始搜尋而不改變可行域，多 seed portfolio 與差異候選則在固定時間內增加取得實用
