...
\begin{tabularx}{\textwidth}{p{2.6cm}X X}
  ...
  \((\lambda_{ed},\ell_{ed})\) & 最近有效正常班別及其連續出現次數；\(R\)、\(R1\)、\(R休\)、\(X\) 都略過，遇到不同正常班才結束 & \texttt{MS03} 同班別區塊 \\
...
\subsubsection{\texorpdfstring{$J^M_2$}{J2}：外援人力}
...
\subsubsection{\texorpdfstring{$J^M_3$}{J3}：每月休假分布}
...
\subsubsection{\texorpdfstring{$J^M_4$}{J4}：排班品質}
...
序列時，\(R\)、\(R1\)、\(R休\) 與 \(X\) 全部略過；只有下一個有效正常班別不同時，
...
\subsubsection{\texorpdfstring{$J^M_5$}{J5}：輪轉與公平}
