...
\begin{itemize}
  \item 目標月份與固定延伸天數 \(K\)；
  \item M 人員及其所屬車站；
  \item 指定休假 \(R^*\)、每人目標月精確 \(R休\) 數、固定勤務 \(X\) 與其他固定指派；
...
在目標月份必須使用的精確 \(R休\) 數，取自需求列的 \texttt{當月指定R休}；
...
\[
  ...
  \qquad
  \sum_{d\in\mathcal{D}^{\mathrm{tar}}}r^{M,L}_{ed}=k_e.
\]
...
指定休假參數 \(p_{ed}\)、每人精確 \(R休\) 數 \(k_e\)、固定勤務參數 \(z_{ed}\)、八週額度
...
\[
  ...
  \qquad
  \sum_{d\in\mathcal{D}^{\mathrm{tar}}}r^{T,L}_{ed}=k_e.
\]
