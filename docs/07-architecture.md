...
> 本文件描述系統邊界與模組責任；實際開發流程與檔案導覽見 `11-implementation-plan.md`。
...
| 規則說明 | 班表警示顯示有意義的規則名稱與繁中原因；硬規則不可關閉，軟規則權重可於求解前調整 |
| 排班服務 | 輸入驗證、M／YM 共用 `MSolver`、T／YT 共用 `TSolver`，依固定 Priority 群組做字典序最佳化、最多 3 份候選 |
...
- Administrator 建立帳號、設定一次性密碼、停用帳號、軟刪除帳號及配置 M/T/YM/YT 工作區編輯權；四者不互相繼承，首次登入或重設後必須改密碼。已登入者可從頁首隨時自行修改密碼，須驗證目前密碼。軟刪除會同時停用帳號、更新 security stamp 並從管理列表隱藏；Identity 與稽核資料保留，帳號名稱不得重新使用。
