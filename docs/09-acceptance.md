...
- 需求編輯器只顯示求解輸入；班表編輯器顯示實際月份日期、56 日區間與月統計，M 底部顯示車站與外派人數。月表 CSV 固定保留 1–31 日，現行完整格式為 48 欄；Web 下載為 M／YM 46 欄、T／YT 47 欄。匯入仍接受無 `萬年班表` 的 47 欄格式、無 `月中排班終止日` 的 47／46 欄舊格式、再缺 R休下限的 46／45 欄舊格式，以及舊欄名 `當月指定R休下界`、`當月指定R休`。
...
- 月中排班終止日當日仍可排班，隔日起日格必須空白且不計出勤或班位；舊 CSV 缺少此欄時視為未設定。
