...
- 首次登入強制修改至少 8 字元、至少 2 種不同字元且包含數字的密碼；之後可從頁首「修改密碼」自行更換，須驗證目前密碼。五次失敗鎖定 15 分鐘，另以帳號與 IP 做登入限流，錯誤訊息不揭露帳號是否存在。
...
- 需求範本不顯示必須留空的求解後統計欄位；歷史班表範本不顯示能力、當月／月底統計與當月指定 R休。匯入仍接受含這些舊欄位的檔案；統計依日格重算，T／YT 能力依工作區與員工 ID 從員工列表帶入，找不到時為 1。月初 R/R1 留空時依前月 `★` 採用班表回推，確實無法回推才阻擋匯入。
- 需求與班表上傳的必填、條件必填、可留空與系統自動產生欄位說明放在各頁右上角 `?` 說明中。
...
- 任何已登入者可在 M/T 填報頁代填任一員工的 R*、R休 上下界、固定班與 X；選取已有填報的員工可讀取並覆蓋，AuditLog 含操作者與被填員工。Editor 在 Demand 寬表預覽填報時，所有可匯入的員工列預設全部勾選，並可逐列調整；確認後只覆蓋已勾選員工的 R休 上下界與日格，未勾選員工的正式 Demand 內容保持不變。超過本月每人 R* 上限的列顯示紅字但仍可匯入。
...
- 需求編輯器只顯示求解輸入；班表編輯器顯示實際月份日期、56 日區間與月統計，M 底部顯示車站與外派人數。月表 CSV 固定保留 1–31 日，現行完整格式為 48 欄；Web 下載為 M／YM 46 欄、T／YT 47 欄。匯入仍接受無 `萬年班表` 的 47 欄格式、無 `月間排班終止日` 的 47／46 欄舊格式、再缺 R休下限的 46／45 欄舊格式，以及舊欄名 `當月指定R休下界`、`當月指定R休`。
...
- 非常態班型 CSV 可讀成 typed table；同代碼以分號分隔的各名稱與代碼都能匯入並保留所選名稱，YM／YT 輸出加上 `(代碼)`；重複、保留名稱或非法定義會失敗。
...
- 月間排班終止日當日仍可排班，隔日起日格必須空白且不計出勤或班位；舊 CSV 缺少此欄時視為未設定。
...
- M/YM 的當月站群、班位最少／最多人數、零需求班位禁排、三種外援等級與跨月夜–休–早／午符合文件；兩者共用同一 `MSolver` 與規則。M 首月相容設定中 LB02／LB04／LB11 為「允許」、LB09 為「盡量不要」；YM 首月固定六組，Y06–Y19 各班 1–1 且禁止外援。
...
- M 先最佳化 J1，再最佳化直接加權合併的 `J4+J5`；非偏好輪轉與跨站支援公平不建模、不輸出，其餘項目與權重符合 `docs/05-soft-rules.md`。
