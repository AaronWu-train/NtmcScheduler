...
- Demand 與候選班表寬表的表頭、欄位順序及每格正規化內容與 CLI 月班表輸出完全相同，固定保留 1–31 日及所有 46 欄；不得另加右側統計欄或底部彙總列。
...
- Demand「上月班表」上傳成功後，同一內容會出現在上月份的班表版本中，且可由原步驟開啟或手動標示 `★`；上傳本身不得自動標星。直接上傳版本沒有上上月歷史時，不對資料不足的月初跨月視窗產生假違規。
