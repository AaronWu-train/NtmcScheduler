...
- M 的站群、班位人數、零需求班位禁排、外派站點與跨月夜–休–早／午符合文件；LB09 早／午可外派且首人即計入 J4，原三站仍共用 60 人次免罰。早、午班可超過最低需求；每站每日夜班至多 1 人，有需求時恰好 1 人。MS03 沿用 CS04 工作區段邊界，區段內出現超過一種正常班型時計 +1。
...
- M 的休假、跨站支援與早／午／夜班數公平只比較同站群整月人員。每群以 `floor(10 × Σ|群組人數 × 個人次數 − 群組總次數| ÷ 群組人數)` 計分，並合計各群；早／午／夜班相對權重為 2:2:30。一般違反量的權重乘以 10，公平性違反量本身則以 0.1 為單位放大 10 倍，兩者加權分數位於相同尺度。
- M 先最佳化 J1，再最佳化直接加權合併的 `J4+J5`；原 J2、J3 的項目與非偏好輪轉仍位於 J4，權重符合 `docs/05-soft-rules.md`。
