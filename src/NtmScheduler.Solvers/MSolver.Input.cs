...
public static partial class MSolver
{
    ...
    private static void ValidateScheduleRows(ScheduleInput input, MonthlySchedule schedule, bool history, List<InputError> errors)
    {
        ...
        string prefix)
    {
        ...
        {
            if (!HasValidWorkEventInterval(date, cell))
                errors.Add(new($"{prefix}.Assignments", $"X {employee.EmployeeId}/{date:yyyy-MM-dd} needs a valid UTC+8 interval of at most 24 hours."));
    ...
    private static IEnumerable<(DateOnly Date, ScheduleCell Cell)> ResolvedHistoryFor(ScheduleInput input, string employeeId)
    {
        ...
        return employee is null
            ? []
            : employee.Assignments.Select(pair => (pair.Key, pair.Value));
    }
    ...
    /// <summary>Returns quota already consumed or credited before this employee's active modeled dates.</summary>
    ...
    private static (DateTimeOffset Start, DateTimeOffset End) NormalShiftInterval(DateOnly date, Shift shift)
    {
        ...
        {
            Shift.Early => (new TimeOnly(6, 30), new TimeOnly(14, 30), false),
            Shift.Afternoon => (new TimeOnly(14, 20), new TimeOnly(22, 20), false),
            Shift.Night => (new TimeOnly(22, 0), new TimeOnly(7, 0), true),
    ...
    private static Shift? HistoricalLastShiftSinceRest(ScheduleInput input, string employeeId)
    {
        ...
        foreach (var item in ResolvedHistoryFor(input, employeeId).OrderBy(item => item.Date))
        {
            if (item.Cell.Kind is AssignmentKind.Rest or AssignmentKind.SpecialRest or AssignmentKind.LeaveRest) lastShift = null;
            else if (item.Cell.Kind == AssignmentKind.Work) lastShift = item.Cell.Shift;
        }
        return lastShift;
    }

    private static Shift? HistoricalLastNormalShift(ScheduleInput input, string employeeId) =>
        ResolvedHistoryFor(input, employeeId)
            .Where(item => item.Cell.Kind == AssignmentKind.Work && item.Cell.Shift is not null)
            .OrderBy(item => item.Date)
            .Select(item => item.Cell.Shift)
            .LastOrDefault();
}
