...
public static partial class ScheduleCsv
{
    ...
    [
        "ID", "姓名", "所屬", "月中開始排班日", "月中終止排班日", "能力", "T月班別",
        "月初區間累計R", "月初區間累計R1",
    ...
    private static readonly string[] OldLegacyHeaders =
        LegacyHeaders.Where(x => x is not "R休下限" and not "月中終止排班日").ToArray();
    private static readonly string[] OldHeaders = [.. OldLegacyHeaders, "萬年班表"];
    private static readonly string[] PreviousLegacyHeaders = LegacyHeaders.Where(x => x != "月中終止排班日").ToArray();
        ...
        bool ignoreDerivedHistoricalFields)
    {
        ...
        var employee = new EmployeeMonthlySchedule
        {
            ...
            EmploymentEndDate = NullableDate(row[4], $"{field} 月中終止排班日"),
            Ability = ability,
