    ...
    ILogger<ScheduleRunWorker> logger) : BackgroundService
{
    ...
    private static async Task StoreMResultAsync(NtmDbContext db, ScheduleRun run, MonthlySchedule demand, MSolveResult result, CancellationToken cancellationToken)
    {
    ...
    private static async Task StoreTResultAsync(NtmDbContext db, ScheduleRun run, MonthlySchedule demand, TSolveResult result, CancellationToken cancellationToken)
    {
        run.Status = Map(result.Status);
        run.Error = ErrorText(result.Errors);
        var configurationId = await ConfigurationIdAsync(db, run.DemandDraftId, cancellationToken);
    ...
    private static async Task CompleteAsync(NtmDbContext db, ScheduleRun run, int candidateCount, CancellationToken cancellationToken)
    {
    ...
    private static async Task<Guid> ConfigurationIdAsync(NtmDbContext db, Guid demandId, CancellationToken cancellationToken) =>
